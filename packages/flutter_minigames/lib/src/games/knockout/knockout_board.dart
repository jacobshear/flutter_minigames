...
class KnockoutScene extends FlameGame {
  ...
  final Map<String, KnockoutSlideSpin> _spin = {};
  ...
  void _integrateSpin() {
    for (final d in _sim.discs) {
      ...
      if (prev != null) {
        (_spin[d.id] ??= KnockoutSlideSpin(d.id))
  ...
  void render(Canvas canvas) {
    ...
    final pucks = <KnockoutRenderPuck>[];
    for (final d in _sim.discs) {
      if (d.removed) continue;
      final n = _toNorm(d.position);
    ...
    final impacts = <KnockoutImpactRing>[
      for (final i in _impacts)
        KnockoutImpactRing(
          nx: i.nx,
          ny: i.ny,
...
/// [KnockoutFallingPuck] each frame from [knockoutFallFrame].
class _Falling {
  final KnockoutFallEdge edge;

  /// Position along the exiting lip (nx for [KnockoutFallEdge.top]/[KnockoutFallEdge.bottom];
  /// ny for the sides).
  final double along;
...
/// One live contact pulse (scene-private; the painter gets an [KnockoutImpactRing]).
class _Impact {
  final double nx;
  final double ny;
...
class KnockoutSlideSpin {
  ...
  KnockoutSlideSpin(String id) : _dir = (id.hashCode & 1) == 0 ? 1.0 : -1.0;
...
///    cylindrical side wall comes into view ([KnockoutFallingPuck.rim]), which is what
...
///    the dark and fades.
KnockoutFallingPuck knockoutFallFrame({
  required KnockoutFallEdge edge,
...
enum KnockoutFallEdge { top, bottom, left, right }

/// A puck to draw, in normalized platform coords.
class KnockoutRenderPuck {
  final double nx;
  final double ny;
  ...
  /// Accumulated facing in radians (see [KnockoutSlideSpin]). The markings turn through
...
void _paintImpact(Canvas canvas, Offset c, double r, KnockoutImpactRing i) {
...
void _paintPuck(Canvas canvas, Offset c, double r, KnockoutRenderPuck p) {
  if (r <= 0.4) return;
