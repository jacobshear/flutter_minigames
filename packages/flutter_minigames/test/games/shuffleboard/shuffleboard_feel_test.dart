...
void main() {
  group('ShuffleboardSlideSpin', () {
    test('accumulates with distance travelled, not with number of steps', () {
      final a = ShuffleboardSlideSpin('p1-0');
      // One 2-unit hop.
      a.advance(2, 0, 0.5);
      final oneHop = a.angle.abs();

      final b = ShuffleboardSlideSpin('p1-0');
    ...
    test('grows monotonically with further travel', () {
      final s = ShuffleboardSlideSpin('p1-0');
      var last = 0.0;
      for (var i = 0; i < 8; i++) {
    ...
    test('direction is stable per puck, and pucks differ', () {
      final a = ShuffleboardSlideSpin('p1-0')..advance(1, 0, 0.5);
      final b = ShuffleboardSlideSpin('p1-0')..advance(1, 0, 0.5);
  ...
  group('fall off a lip', () {
    ...
    test('each lip sends the puck out its own side', () {
      expect(at(1, edge: ShuffleboardFallEdge.top).ny, lessThan(0));
      expect(at(1, edge: ShuffleboardFallEdge.left).nx, lessThan(0));
      expect(at(1, edge: ShuffleboardFallEdge.right).nx, greaterThan(1));
      // A side fall foreshortens across, not down-lane.
      expect(at(0.32, edge: ShuffleboardFallEdge.left).squashX, lessThan(0.45));
      expect(at(0.32, edge: ShuffleboardFallEdge.left).squashY, 1);
    });
  });
}
