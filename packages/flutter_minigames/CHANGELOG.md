...
### Replay, fast-forward and polish (also in 0.1.0)

- `MatchController.submitMove` never throws for a refused write: it returns
  false and reports the error on `turnRejected` (and each accepted write on
  `turnCommitted`). Boards call `submitMove` unawaited, so a thrown transport
  error used to surface as an unhandled async error no host could catch.
