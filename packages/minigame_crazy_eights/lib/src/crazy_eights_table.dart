...
class _CrazyEightsTableState extends State<CrazyEightsTable>
  ...
  late final AnimationController _drawFly = AnimationController(
    vsync: this,
  ...
  ({Offset center, double angle}) _fanSlot(
    ...
    final pivot = Offset(size.width / 2, size.height * 0.82 - cardH * 0.25 + r);
    return (
  ...
  Offset _stockCenter(Size size, double cardW) =>
      Offset(size.width / 2 - cardW * 0.72, size.height * 0.44);
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        gradient: RadialGradient(
          ...
          radius: 1.5,
          colors: [
            Color.lerp(table, Colors.white, 0.08)!,
            table,
            Color.lerp(table, Colors.black, 0.28)!,
          ],
  ...
  List<Widget> _buildOpponentRow(
    ...
    for (var i = 0; i < count; i++) {
      ...
      final start = Offset(size.width / 2, size.height * 0.44);
    ...
    widgets.add(Positioned(
      ...
      child: Transform.scale(
        ...
        child: Opacity(
          ...
          child: PlayingCardView(card: topCard, width: cardW),
        ),
      ),
    ));
    ...
    if (declared != null && _playAnim == null) {
      ...
      widgets.add(Positioned(
        ...
        child: Container(
          ...
          child: CustomPaint(
            painter: _SuitBadgePainter(suit: declared),
          ),
        ),
      ));
    }
  ...
  List<Widget> _buildFan(
    ...
    final deckC = Offset(size.width / 2, size.height * 0.44);
  ...
  Widget _buildFlyingDraw(
    ...
    if (toBottom) {
      ...
      to = Offset(size.width / 2, size.height * 0.145);
      angle = 0;
    }
    final at = Offset.lerp(from, to, t)!;
    return Positioned(
      ...
      child: IgnorePointer(
        child: Stack(
          children: [
            ...
            // Above the fan so the hand never buries it.
            Positioned(
              left: 12,
  ...
  Widget _buildOutcomePill(CrazyEightsState state) {
    final outcome = _outcome;
    String? msg;
    if (outcome != null) {
      if (outcome.isDraw) {
        ...
        final label = widget.style.seatLabel(seat).toUpperCase();
  ...
  List<_Confetto> _spawnConfetti() {
    const palette = [
      Color(0xFFF4B740),
      Color(0xFFFFFFFF),
      CrazyEightsCardArt.red,
      CrazyEightsCardArt.backBlue,
    ];
