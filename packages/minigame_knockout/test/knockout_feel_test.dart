...
void main() {
  group('SlideSpin', () {
    test('accumulates with distance travelled, not with number of steps', () {
      final a = SlideSpin('p1-0')..advance(2, 0, 0.5);
      final b = SlideSpin('p1-0');
      for (var i = 0; i < 20; i++) {
        b.advance(0.1, 0, 0.5);
      }
      expect(b.angle.abs(), closeTo(a.angle.abs(), 1e-9));
      expect(a.angle.abs(), closeTo(2 / 0.5 * SlideSpin.turnsPerRadius, 1e-9));
    });
    ...
    test('grows monotonically with further travel', () {
      ...
      expect(last, closeTo(4 / 0.5 * SlideSpin.turnsPerRadius, 1e-9));
    });

    test('a puck that has not moved does not turn', () {
      final s = SlideSpin('p1-0')..advance(0, 0, 0.5);
      expect(s.angle, 0);
    });
  ...
  group('fall off a lip', () {
    ...
    test('each lip sends the puck out its own side', () {
      ...
      expect(at(1, edge: FallEdge.bottom).ny, greaterThan(1));
      expect(at(1, edge: FallEdge.left).nx, lessThan(0));
      expect(at(1, edge: FallEdge.right).nx, greaterThan(1));
      expect(at(0.30, edge: FallEdge.left).squashX, lessThan(0.45));
      expect(at(0.30, edge: FallEdge.left).squashY, 1);
    });
  });
